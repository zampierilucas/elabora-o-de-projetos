...
Diariamente milhares de Kerneis Linux são compilados. Tal compilação requer maquinas cada vez mais capazes e eficientes. Empresas compilam Kerneis para distribuição em sistemas integrados, \citetexto{huong2022development}, uso servidores e uso pessoal, entre elas podemos citar Intel, Red Hat, Linaro e IBM, como demonstrado em \citetexto{linuxfoundation_2017}.

Atualmente empresas que compilam Kerneis Linux possuem seus próprios ambientes virtualizado para compilação, tais ambientes se beneficiam de diversas formas de compilação, entre as já exploradas virtualização, para-virtualização, virtualização hibrida, contêineres e bare-metal.
...
Analisar os custos ou benefícios de desempenho na compilação-cruzada  em relação a  plataformas x86, Arm e s390 no contexto da compilação do Kernel Linux.
...
A pesquisa se diferencia dos artigos citados, pois foca apenas no aspecto das arquiteturas envolvidas sem considerar virtualizações de alto nível, como \citetexto{giallorenzo2021virtualization}. Enquanto \citetexto{nakajima2011optimizing} foca em otimizações da pilha de software, essa pesquisa foca na otimização de compilação conforme o hardware envolvido.
